...
class MediaControlApp(ctk.CTk):
    def __init__(self):
        ...
        self.artist_label = ctk.CTkLabel(self.info_frame, text="Waiting...", font=("Segoe UI", 11), text_color="#888", anchor="w")
        ...
        self.play_btn = ctk.CTkButton(ctrl_frame, text="⏯", width=45, fg_color="#1DB954", text_color="black", command=lambda: self.media_cmd("play_pause"))
        ...
        self.visual_trigger.place(x=0, y=0) # Only half shows on edge
    ...
    async def _update_ui(self):
        try:
            ...
            if session:
                ...
                self.title_label.configure(text=props.title[:25])
                self.artist_label.configure(text=props.artist[:30])
